...
<div class="px-6 py-4 space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-semibold text-white">{{ __('Edit Game') }}</h1>
  ...
  <div class="bg-neutral-900 border border-neutral-800 rounded p-6">
    <form method="post" action="{{ route('admin.tournaments.games.update', [$tournament, $game]) }}" class="space-y-6" novalidate enctype="multipart/form-data">
      @csrf
      @method('put')
      @include('admin.tournaments.games._form')
      <div class="flex justify-end">
        <button type="submit" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded">{{ __('Update Game') }}</button>
      </div>
    </form>
  </div>
</div>
